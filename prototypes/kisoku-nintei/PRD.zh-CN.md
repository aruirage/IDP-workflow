...
| 自动整理并全体表示 | 整列して全体表示     | 画布        | 存在节点            | 重布局并重算连线路径，然后调整视口让画布内容完整显示                    |
...
| 自定义函数输入参数 | 弹窗+列表  | 按用户添加  | 用户添加的输入参数；变量选择器与条件节点一致，并额外允许开始节点 `caseId`、`files[]` | 参数名唯一；Source 不可选后续节点 | 保存为 Python 节点入参映射；脚本内用自定义变量名读取入参；脚本返回值只进执行日志，不进变量池                                             |
...
节点定位：用户为 Python 脚本添加输入参数，并把每个参数绑定到变量选择器里的变量。自定义节点变量选择器与条件节点一致，唯一差异是额外允许选择开始节点输出变量。脚本返回值只进执行日志，不进入条件、系统、待办变量池。下游节点只能消费本节点内置 `codeStatus`。

入参规则：

1. 右侧 inspector 的「入力変数」区只展示已添加入参列表；点击「追加」打开入参弹窗。
2. 入参弹窗填写变量名、是否必填，并从 Source 下拉选择一个数据来源；不配置数据类型。
3. 变量名由配置人员自定义，作为 Python 脚本读取键；同一自定义节点内不可重复。
4. 添加完成后，入参列表展示变量名、必填标记和绑定 Source；每条支持编辑和删除。
5. Source 选择器展示规则与条件变量一致：一级只显示节点名称或账票类型名称，二级只显示裸变量键或字段名，不展示内部节点标识。
6. 脚本内用 `inputs['变量名']` 读取已定义输入变量；例如变量名为 `claimFiles` 时，用 `inputs['claimFiles']` 读取。变量类型由脚本自行判断和转换，平台不在入参弹窗里要求选择类型。
...
| 条件节点可选变量    | 与条件节点变量选择器一致           | 包含上游可达节点输出、Step1 OCR 字段叶子、标准字段叶子等；不额外开放文件类别 JSON |

...
| AI 前处理能力  | 开关+多选  | 至少一项   | 画像旋转/补正/分割/整列/组合；画像组合对象为 Step1 关联账票中已在账票类型设置启用画像组合的账票 | 全关阻断保存；画像组合下拉不展示未启用画像组合的账票类型                         | 节点仅保存是否执行画像组合及对象账票，组合必须键、组合任意键沿用账票类型设置；运行时输出 `preprocessStatus`、`preprocessResult`；条件节点只消费 Result，Step3 通知可消费 Status / Result |
...
| 面板标题  | AI前処理                        | 右侧检视面板顶栏 |
| 区块标题  | 前処理能力                        | 多能力开关    |
| 能力项   | 画像回転 / 画像補正 / 画像分割 / 画像整列 / 画像組合 | 可单独开关；画像組合默认关闭；对象账票和组合键引用账票类型设置 |

画像組合开启后，对同一案件内、同一账票类型的图片按账票类型设置中的组合必须键和组合任意键判断是否合并为一个逻辑文件。工作流节点不重复编辑组合键。对象账票仅展示 Step1 已关联且在账票类型设置中启用画像組合的账票类型；未启用的账票类型不可选择。

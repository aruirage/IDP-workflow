...
3. Step2：进入工作流画布；画布读取 Step1 账票与关系，限制节点可选范围。独立的设置检查执行静态配置校验；不读取测试样例、不调用节点。
...
6. 发布：Step2 设置检查通过后，场景变为可发布。Step3、Step4 只保存通知和输出配置，不参与发布门禁。Step4 点击公开时若不是可发布状态则自动执行同一检查；检查通过后显示确认弹窗，确认后生成新版本号；新案件只读取已发布版本。
...
| 清空   | リセット        | Step2 左下角工具栏 | 有编辑权限                       | 位于变更历史右侧；弹窗确认后删除开始节点以外的所有工作流节点和连线，场景回草稿                         |
| 设置检查 | 設定チェック     | Step2   | 有编辑权限                       | 执行静态配置校验；失败时提示首条错误并定位节点，成功时更新为可发布                           |
| 下一步  | 次へ          | Step2→3 | 始终可用                       | 进入 Step3 通知设置，不执行设置检查                                      |
...
| 发布   | 公開          | Step4   | 始终可点击                       | 非可发布状态先执行 Step2 设置检查；失败时提示并跳转 Step2，成功时显示公开确认                  |
...
| 场景状态        | 只读标签    | 是  | 草稿、可发布、已发布                 | 草稿=新建、复制或 Step1/Step2 有实质变更；可发布=Step2 设置检查通过；已发布=当前查看正式版本                                 | Step1～4 工具栏、上传过滤 |
...
| 显示状态 | 实现取值        | 触发条件                                                                | 按钮影响                                    |
| ---- | ----------- | ------------------------------------------------------------------- | --------------------------------------- |
| 草稿   | `draft`     | 新建/复制后；Step1/Step2 存在实质变更；旧 `pending_review` 读档时回退为草稿 | Step2 设置检查通过前不可公开                         |
| 可发布  | `ready`     | Step2 设置检查通过                                                 | 可公开；允许继续编辑；编辑 Step1/Step2 后回草稿          |
...
| Step2 保存成功 | 画布、节点、连线、变量引用    | 不改变   | 只保存当前草稿，不执行设置检查                 |
| Step2 设置检查通过 | 当前画布配置          | 可发布   | 执行静态配置校验并更新状态                    |
...
已发布后再编辑：已发布版本只读。用户从版本履历切换到「現在の下書き」后继续编辑；正式案件仍使用上一个已发布版本。草稿再次通过 Step2 设置检查、再确认公开后，生成新版本并替换后续新案件使用的版本。
...
Step2 不提供样例测试入口，提供独立的设置检查。设置检查执行静态配置校验，检查结构、连线、节点必填项、变量引用、字段选择和节点依赖；不读取 Mock 样例、不求值条件路径、不真实调用节点。检查失败时提示首条错误；错误对应节点时选中并定位该节点。保存、下一步和步骤条跳转不触发该检查。

Step2 清空删除开始节点以外的所有工作流节点和连线。开始节点是系统固定入口，默认存在，节点面板不提供新增，用户不能单独删除，也不会出现多个开始节点。リセット 后场景回到草稿。清空后画布仅保留开始节点，用户需要重新添加处理节点、条件节点、人工确认和结束节点，并重新通过设置检查后才能公开。
...
Step4 没有测试。点击「保存」固化当前勾选并记为已保存；保存不改写 Step1～Step3，也不重验工作流。保存与公开均使用蓝底主按钮。点击公开时，若场景不是可发布状态，系统直接执行 Step2 设置检查；失败时用 Toast 提示首条原因并自动跳转 Step2，成功时更新为可发布并继续显示公开确认。发布成功后生成新版本号并标记已发布。
...
| S4-04 | 公开       | 主状态为可发布；不是可发布时自动执行 Step2 设置检查                                                                 | 检查失败时 Toast 提示首条原因并自动跳转 Step2；检查通过时更新状态并显示公开确认弹窗；Step4 是否保存不阻断发布                  |

#### 6.01.10 步骤跳转、引用保护与联动

Step1 提供关联检查；Step2 负责工作流画布配置，并通过独立的设置检查更新可发布状态；Step3 保存通知配置；Step4 保存输出勾选并发起公开。保存、下一步和步骤条跳转均不触发 Step2 设置检查。点击 Step4 的公开时，若当前不是可发布状态则自动执行同一检查；通过后显示确认弹窗，确认后直接公开。
...
| Step2 | Step3 | 无阻断条件 | 直接进入 Step3；不执行设置检查、样例测试或节点调用             |
| Step3 | Step4 | 点击「次へ」或已保存后继续  | Step3 保存的是通知规则模板；Step4 读取 Step2 OCR 抽出候选    |
| Step4 | 公開    | 按钮始终可点击        | 当前不是可发布时先执行 Step2 设置检查；失败则提示并跳转 Step2，成功则显示公开确认             |
...
Step1～4 顶部工具栏在场景名后同步展示同一状态徽章（下書き / 公開可能 / 公開済み）。已发布过的场景在顶部展示版本切换图标。下拉列表只包含「現在の下書き」和最新发布版本；发布版本标记为适用中。发布成功后立即显示最新发布版本。选择发布版本后，Step1～4 的表单、画布和快捷操作全部只读，但保留步骤条及「前へ / 次へ」用于浏览四步配置；隐藏关联检查、保存和公开等编辑操作。未主动切换到「現在の下書き」前不能编辑；切换后恢复草稿编辑。再次发布时覆盖最新发布版本，不展示或保留可切换的更早版本。列表工具栏关键词检索，占位「業務シーン名で検索」。无数据时展示空状态；无权限隐藏新建。

#### 6.01.12 异常与边界

| 场景           | 条件                    | 用户提示 / 处理                                                  |
| ------------ | --------------------- | ---------------------------------------------------------- |
| Step2 设置检查失败   | 点击 Step2 设置检查，或在非可发布状态点击公开 | Toast 提示首条错误并定位对应节点；从 Step4 发起时自动跳转 Step2；场景保持草稿                            |
...
| 公开前自动检查       | 场景不是可发布且点击公开          | 直接执行 Step2 设置检查；失败时 Toast 提示首条原因并自动跳转 Step2，成功时继续显示公开确认 |
...
| 删除节点/连线   | 削除 / この接続を削除 | 选中项       | 有编辑权限；选中项不是开始节点 | 节点不弹确认，直接删除选中节点及相关连线；开始节点不能单独删除；删除后场景回到草稿                   |
...
| 清空        | リセット        | 左下角工具栏    | 有编辑权限           | 位于变更历史右侧；弹窗确认后删除开始节点以外的全部节点和连线，保留固定开始节点并置为未测试       |
| 自动整理并全体表示 | 整列して全体表示     | 画布        | 存在节点            | 按左到右层级重布局，自动疏散节点以保留连线曲率空间，重算连线路径，然后调整视口让画布内容完整显示 |
| 设置检查      | 設定チェック      | 画布        | 有编辑权限           | 执行静态配置校验；通过后更新为可发布，失败时提示首条原因并定位节点             |
| 保存        | 保存           | 画布        | 有编辑权限           | 保存当前草稿，不执行设置检查                                 |
| 前へ        | 前へ           | 向导        | 始终可用            | 返回上一步                                            |
| 次へ        | 次へ           | Step2     | 始终可用           | 进入 Step3，不执行设置检查                                  |
| 公開        | 公開           | Step4     | 始终可点击          | 非可发布状态先执行 Step2 设置检查；失败时提示并跳转 Step2，成功时显示公开确认       |
...
Step2 点击「設定チェック」时执行静态配置校验；当前状态不是可发布而点击 Step4「公開」时，也执行同一校验。校验只读取当前画布、Step1 账票与字段定义；不读取样例数据，不执行 OCR、AI 检证、数据映射或自定义 JavaScript，不创建人工待办，不发送通知或邮件。
...
校验失败时场景保持草稿；从 Step4 发起检查时，Toast 提示首条原因并自动跳转 Step2。校验通过时保存当前检查结果，场景更新为可发布；从 Step4 发起检查时继续显示公开确认。Step1 或 Step2 后续发生实质修改时，场景重新变为草稿，必须再次通过设置检查。

公开不执行样例测试。Step4 的公开按钮始终可点击；当前不是可发布状态时先执行 Step2 设置检查，检查通过后显示公开确认弹窗，确认后直接生成发布版本并切换为只读。

...
| 关联账票字段关系    | 下拉组     | 是  | 关联账票 OCR 字段                | 按帐票对配置；同一账票类型存在多种版式时，每种版式配置一个条件组，并按 A、B、C 顺序标识；每个条件组至少 1 条字段关系；组内固定按 AND 判断，组间固定按 OR 判断；字段不存在或为空时，该条字段关系及所属条件组不成立；字段类型从账票类型设置读取 | 案件集约             |
...
同一帐票对的组间逻辑固定为 OR。任一版式对应的条件组成立，即可判定该帐票对关联成功。界面在相邻条件组之间显示只读 OR 标记，并在每个条件组标题旁显示「グループ内：AND」。

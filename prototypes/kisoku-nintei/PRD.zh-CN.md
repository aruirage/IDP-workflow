...
| 关联账票字段关系    | 下拉组     | 是  | 关联账票 OCR 字段                | 每关联账票至少一组；字段类型从账票类型设置读取                                                                                 | 案件集约             |
...
| 4 | ノード設定 | 自定义函数必須入参取值 | 必須の入力変数を取得できません | 自定义函数节点存在必須入参，但测试运行时该入参为空、缺失或参照源不可解析 | 对应节点失败；停止本轮 |
...
节点定位：用户为 JavaScript 添加输入参数，并把每个参数绑定到变量选择器里的变量。自定义节点变量选择器与条件节点一致，唯一差异是额外允许选择开始节点输出变量。主要用途是让客户 DIY 输出文件名等命名逻辑；代码返回值只进执行日志或本节点调试，不进入条件、系统、待办变量池。下游节点只能消费本节点内置 `codeStatus`。

入参规则：

「入力変数」弹窗的意义是声明 JavaScript 会读取哪些上游变量。它不是新建变量池变量，也不是复制上游变量；参照元变更或删除时，该入参会失效，并在保存、测试或发布校验中提示。

| 作用       | 说明                                                             |
| -------- | -------------------------------------------------------------- |
| 多变量声明    | 一个自定义函数可能同时读取案件 ID、文件列表、标准字段、OCR 字段等多项输入，弹窗用于逐条声明这些输入 |
...
| 权限与范围控制  | JavaScript 只能读取已声明入参和平台白名单注入参数，不能任意读取全部变量池或直接读取环境变量     |

1. 右侧 inspector 的「入力変数」区只展示已添加入参列表；点击「追加」打开入参弹窗。
2. 入参弹窗填写变量名、必須开关，并从 Source 下拉选择一个数据来源；不允许手工选择数据类型。
3. 变量名由配置人员自定义，作为 JavaScript 读取键；同一自定义节点内不可重复。字段级变量的显示名可能是日文，因此允许用英文变量名做代码内参照名。
4. 添加完成后，入参列表展示变量名、必須标记和绑定 Source；每条支持编辑和删除。
...
| 弹窗字段  | 必須           | 入参值校验开关；开启后运行时没有值则不执行脚本并失败，关闭后没有值也继续执行脚本 |
| 区块标题  | JavaScript     | 代码编辑区；代码通过 `inputs.变量名` 或 `inputs["变量名"]` 使用已添加输入变量 |
...
| 必須开关       | 开启时，运行时入参为空、缺失或 Source 不可解析，节点不执行脚本，写 `codeStatus=failed` 并记录执行日志；关闭时，运行时把 `null` 传给 JavaScript，脚本继续执行并自行兜底 |
| 代码输入固定     | JavaScript 用 `inputs.变量名` 或 `inputs["变量名"]` 读取已定义输入变量             |
| 环境变量读取      | 不允许直接读取环境变量；运行环境路径、租户代码等环境级参数只能由平台白名单注入                  |
| 代码返回值不生成变量 | 返回值用于本节点执行结果、日志和调试预览，不进入任何变量池                                  |
| 返回值校验       | 绑定到具体用途时，平台按用途校验返回值。例如文件名变更用途要求返回字符串或 `{ fileId, fileName }` 数组；缺少必要字段时写 `codeStatus=failed` |
| 条件可选限制     | 条件节点只展示本节点内置 `codeStatus`，不展示代码返回字段                             |
...
const caseId = inputs.caseId || file.caseId || "UNKNOWN_CASE";
const docType = file.classificationResult || "unknown";
const ext = file.extension || ".pdf";

return `${safe(caseId)}_${safe(docType)}_${safe(file.id)}${ext}`;
```
...
return files.map((file, index) => {
  ...
  const caseId = file.caseId || inputs.caseId || "UNKNOWN_CASE";
  const docType = file.classificationResult || "unknown";
  const ext = file.extension || ".pdf";

  return {
    fileId: file.id,
    fileName: `${safe(caseId)}_${seq}_${safe(docType)}${ext}`
  };
});
```
...
| 变量键          | 类型   | 消费分类 | 取值范围                            | 定义                               |
| ------------ | ---- | ---- | ------------------------------- | -------------------------------- |
| `codeStatus` | Enum | 条件   | `processing / success / failed` | JavaScript 处理状态；不输出 result 或错误消息变量 |

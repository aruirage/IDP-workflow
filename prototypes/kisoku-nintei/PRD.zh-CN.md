...
<ins class="change-added">已发布后再编辑：已发布版本只读。用户从版本履历切换到「現在の下書き」后继续编辑；正式案件仍使用上一个已发布版本。草稿再次通过 Step2 设置检查、再确认公开后，生成新版本并替换后续新案件使用的版本。</ins>
...
Step1～4 顶部工具栏在场景名后同步展示同一状态徽章（下書き / 公開可能 / 公開済み）。<ins class="change-added">已发布过的场景在顶部展示版本切换图标。下拉列表只包含「現在の下書き」和最新发布版本；发布版本标记为适用中。发布成功后立即显示最新发布版本。选择发布版本后，Step1～4 的表单、画布和快捷操作全部只读，但保留步骤条及「前へ / 次へ」用于浏览四步配置；隐藏关联检查、保存和公开等编辑操作。未主动切换到「現在の下書き」前不能编辑；切换后恢复草稿编辑。再次发布时覆盖最新发布版本，不展示或保留可切换的更早版本。</ins>列表工具栏关键词检索，占位「業務シーン名で検索」。无数据时展示空状态；无权限隐藏新建。

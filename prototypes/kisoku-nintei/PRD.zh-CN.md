...
| 清空   | リセット        | Step2   | 已进入 Step2               | 删除所有工作流节点和连线；测试结论置为未测试；场景回草稿                         |
...
| Step2 | 所有工作流节点和连线                    | Step1 账票定义、Step3 通知规则、Step4 输出配置       | Step2 测试结论置为未测试；场景回草稿       | 清空后画布为空，用户需要重新添加开始节点和后续节点 |
...
Step2 提供工作流测试，测试结论是发布门禁，不是进入 Step3 的门禁。测试目标是按配置好的工作流，用系统内置测试样例跑一遍，确认当前工作流能否从开始跑到结束。除人工确认待办、通知发送和邮件发送等副作用外，处理节点按当前配置真实调用；测试不创建正式案件、不写真实待办。测试未通过也允许进入 Step3/Step4 继续配置，但场景保持草稿且不可发布；画布、节点配置、连线或变量引用发生实质变更后，测试结论作废。

Step2 清空删除所有工作流节点和连线，包括开始节点。清空后画布为空，用户需要重新添加开始节点、处理节点、条件节点、人工确认和结束节点后才能测试通过。
...
| Step1 修改匹配策略       | 允许；不破坏引用                                            | 允许；Step2 工作流测试结论置为未测试                                       | 保存成功；测试样例按新策略重新测试                        |
...
6. 人工确认：默认模板提供 3 个人工确认，分别接在前处理条件、OCR 条件、AI 检证条件的「要确认」分支之后；校验时穿透条件节点检查最近上游处理节点是否为前处理、OCR 抽出或 AI 检证。每个人工确认只接需要的出口，至少一条。
7. 工作流测试：用内置集约完成测试样例按当前画布跑一遍；同时检查工作流结构、节点配置、变量引用和路径可达性。测试不跑上传/集约，不创建正式案件，不生成真实人工待办，不发送真实通知；OCR、AI 检证、数据映射、自定义 Python 等处理节点按当前配置真实调用。测试成功后允许发布；测试失败不阻断进入 Step3/Step4，但阻断发布。
...
| 删除节点/连线   | 削除 / この接続を削除 | 选中项       | 有编辑权限         | 删除选中节点及相关连线，允许删除开始节点；删除后置未测试       |
...
Step2 画布用工作流测试证明「当前画布配置能按测试样例跑通」。测试通过是发布门禁，不是进入 Step3 的门禁。测试不是正式跑件，也不是业务测试用例管理：它使用测试样例真实调用处理节点，但不创建正式案件、不生成真实人工待办、不发送真实通知、不发送邮件、不证明业务规则覆盖所有现实场景。
...
2. 系统按「结构与配置校验 → 测试样例适配 → 节点真实调用 → 发布门禁写入」顺序处理；前一层阻断则不进入后一层。结构与配置校验是测试执行前的必要检查，包含开始/结束、连线、节点必填项、变量引用、人工确认上游和出口。
3. 执行路径按测试样例求值后的命中路径生成时间线。条件节点按表达式求值选择分支；OCR、AI 检证、数据映射、自定义 Python 等处理节点按配置真实调用；人工确认节点使用测试样例里的人工判断结果选择完成、补件或案件终止出口，不创建真实待办、不等待真实审查者。任一节点失败立即停止，后续节点保持未执行。
...
flowchart LR
  ...
  staticCheck --> adaptInput["适配内置测试样例"]
  adaptInput --> execute["真实调用处理节点"]
...
| 通知 / 邮件发送 | 不发送真实站内信，不发送邮件；只记录应触发的通知规则和渲染结果             |
| 上传 / 案件集约 | 不重新跑上传和案件集约；测试样例视为已完成案件集约                |
| 业务覆盖率     | 不证明所有业务情景都被测试样例覆盖                       |

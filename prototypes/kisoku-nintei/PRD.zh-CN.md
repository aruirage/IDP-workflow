...
flowchart LR
  ...
  draft --> staticFail["Step2 静态校验失败"]
  staticFail --> draft
  draft --> save2["Step2 静态校验通过并保存"]
  save2 --> ready["可发布 ready"]
...
| S4-04 | 公开       | 主状态为可发布                                                                                               | 未满足时提示保存 Step2；满足时显示公开确认弹窗；Step4 是否保存不阻断发布                  |
...
Step1～4 顶部工具栏在场景名后同步展示同一状态徽章（下書き / 公開可能 / 公開済み）。已发布过的场景在顶部展示版本切换图标。下拉列表只包含「現在の下書き」和最新发布版本；发布版本标记为适用中。选择发布版本后，Step1～4 均只读；选择当前草稿后恢复编辑。再次发布时覆盖最新发布版本，不展示或保留可切换的更早版本。列表工具栏关键词检索，占位「業務シーン名で検索」。无数据时展示空状态；无权限隐藏新建。

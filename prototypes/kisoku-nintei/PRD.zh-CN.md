...
| <ins class="change-added">设置检查</ins> | <ins class="change-added">設定チェック</ins>     | Step2   | 有编辑权限                       | <ins class="change-added">执行静态配置校验；失败时提示首条错误并定位节点，成功时更新为可发布</ins>                           |
...
| 连线   | 同一节点仅允许一条普通输入连线、条件出口、人工确认出口、案件中止不连线、补件只回流、悬空连线            | 同上     |
...
4. 变量引用：条件仅选可达上游变量；自定义函数通过「入力変数」追加入参，变量选择器与条件节点一致，并额外允许选择开始节点输出变量。自定义函数不纳入设置检查，脚本及入参问题在运行时记录。
5. 分支：条件 IF/ELIF/ELSE；人工确认三出口（完了/补件/案件中止），审查者选出口即路由。
6. 人工确认：默认模板提供 3 个人工确认，分别接在前处理条件、OCR 条件、AI 检证条件的「要确认」分支之后；完了出口回到对应后续主流程，补件出口只允许回流到补件后的再处理入口，案件中止出口只在画布上展示，不开放连接。案件中止只结束当前工作流实例，案件后续可重新执行工作流。校验时穿透条件节点检查最近上游处理节点是否为前处理、OCR 抽出或 AI 检证。
...
| <ins class="change-added">设置检查</ins>      | <ins class="change-added">設定チェック</ins>      | 画布        | 有编辑权限           | <ins class="change-added">执行静态配置校验；通过后更新为可发布，失败时提示首条原因并定位节点</ins>             |
...
| 自定义函数输入参数 | 弹窗+列表  | 按用户添加  | 用户添加的输入参数；变量选择器与条件节点一致，并额外允许开始节点 `caseId`、`files[]` | 弹窗内校验参数名与 Source；不纳入设置检查                               | 保存为 JavaScript 节点入参映射；代码内用自定义变量名读取入参；返回值只进执行日志或本节点调试，不进变量池                                     |
| 自定义函数脚本   | 代码编辑器  | 否      | 用户输入 JavaScript                                     | 不纳入设置检查；运行时异常写执行日志                                | 保存为 JavaScript 节点执行代码；运行时只输出 `codeStatus`，不进入条件变量选择器；Step3 通知不把自定义函数作为対象ノード                    |
...
| 引用有效性 | Step1 删除账票或字段，或画布删除节点、修改连线后，条件变量不存在失效引用 | 同上 | <ins class="change-added">「{ノード名}」に無効な参照があります。設定内容を確認してください。</ins> | <ins class="change-added">“{节点名称}”中存在无效引用，请检查设置内容。</ins> |
...
<mark class="change-modified">条件节点的变量类型、可选字段和运算符匹配，以及人工确认的默认审查角色由配置控件即时保证，不纳入设置检查。案件中止和补件的可连接范围由画布即时限制，也不纳入设置检查。数据映射与 AI 检证节点调用全局配置，不在 Step2 校验输入数据。</mark>
...
「入力変数」弹窗用于声明脚本会读取哪些上游变量。它不是新建变量池变量，也不是配置出参；自定义函数脚本和入参不纳入设置检查，参照元变化导致的问题在运行时记录。
...
| 依赖处理  | 平台保存每条入参的参照元路径；上游节点、字段或连线变更导致参照元不可达时，运行时按缺失值处理并记录日志              |
...
运行规则：自定义函数不参加 Step2 设置检查。运行时按 Source 解析入参值，JavaScript 用 `inputs.变量名` 或 `inputs["变量名"]` 读取。空值、缺失值和业务兜底由脚本处理；编译失败、运行异常、超时或返回格式非法时写执行日志，并输出 `codeStatus=failed`。

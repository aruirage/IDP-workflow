...
Step2 清空删除所有工作流节点和连线，包括开始节点。开始节点不能单独删除，但 Step2 リセット 会作为整步清空动作一并删除；该动作不按单节点删除的后续引用保护阻断，但会让 Step2 测试结论作废，并提示 Step4 需重新保存。清空后画布为空，用户需要重新添加开始节点、处理节点、条件节点、人工确认和结束节点后才能测试通过。
...
6. 人工确认：默认模板提供 3 个人工确认，分别接在前处理条件、OCR 条件、AI 检证条件的「要确认」分支之后；完成出口回到对应后续主流程，补件和案件终止出口接结束。校验时穿透条件节点检查最近上游处理节点是否为前处理、OCR 抽出或 AI 检证。每个人工确认只接需要的出口，至少一条。
...
| 连线     | 分支名（条件/人工确认）                  | 采用左到右层级排列和平滑曲线；自动整列优先横向展开，分支放在源节点右下方 lane，并疏散过近节点，避免短距离边生成大环线；回流才走外侧通道，测试时标注環状パス |
...
节点定位：系统固定入口。案件集约完成时启动 Workflow。零配置，不展示触发事件列表。不能被单独删除；Step2 リセット 作为整步清空时可一并删除。不提供 Step2 条件变量。

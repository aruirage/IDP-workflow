...
3. Step2：进入工作流画布；画布读取 Step1 账票与关系，限制节点可选范围。保存或进入下一步前执行静态配置校验；不读取测试样例、不调用节点。
...
6. 发布：Step2 静态配置校验通过并保存后，场景变为可发布。Step3、Step4 只保存通知和输出配置，不参与发布门禁。Step4 点击公开后显示确认弹窗，确认后生成新版本号；新案件只读取已发布版本。
...
| 下一步  | 次へ          | Step2→3 | 静态配置校验通过                   | 保存 Step2 当前状态后进入 Step3 通知设置；失败时留在 Step2 并提示首条错误                    |
...
| 场景状态        | 只读标签    | 是  | 草稿、可发布、已发布                 | 草稿=新建、复制或 Step1/Step2 有未保存变更；可发布=Step2 静态配置校验通过并保存；已发布=当前查看正式版本                                 | Step1～4 工具栏、上传过滤 |
...
| 显示状态 | 实现取值        | 触发条件                                                                | 按钮影响                                    |
| ---- | ----------- | ------------------------------------------------------------------- | --------------------------------------- |
| 草稿   | `draft`     | 新建/复制后；Step1/Step2 存在未保存变更；旧 `pending_review` 读档时回退为草稿 | Step2 静态校验通过前不可公开                         |
| 可发布  | `ready`     | Step2 静态配置校验通过并保存                                                 | 可点击公开；允许继续编辑；编辑 Step1/Step2 后回草稿          |
...
flowchart LR
  ...
  save12 --> draft
  draft --> saveFail["Step2 保存失败"]
  saveFail --> draft
  draft --> saveSuccess["Step2 保存成功"]
  saveSuccess --> ready["可发布 ready"]
...
| Step2 保存成功 | 画布、节点、连线、变量引用    | 可发布   | 静态配置校验通过后保存并更新状态                 |
...
已发布后再编辑：已发布版本只读。用户从版本履历切换到「現在の下書き」后继续编辑；正式案件仍使用上一个已发布版本。草稿再次通过 Step2 静态配置校验并保存、再确认公开后，生成新版本并替换后续新案件使用的版本。
...
Step2 不提供单独测试入口。点击保存或下一步时，系统只执行静态配置校验，检查结构、连线、节点必填项、变量引用、字段选择和节点依赖；不读取 Mock 样例、不求值条件路径、不真实调用节点。静态校验失败时不保存、不进入 Step3，提示首条错误；错误对应节点时选中并定位该节点。

Step2 清空删除开始节点以外的所有工作流节点和连线。开始节点是系统固定入口，默认存在，节点面板不提供新增，用户不能单独删除，也不会出现多个开始节点。リセット 后场景回到草稿。清空后画布仅保留开始节点，用户需要重新添加处理节点、条件节点、人工确认和结束节点，并通过静态配置校验后才能保存。
...
| 规则 ID | 检查项      | 通过条件                                          | 失败条件 / 处理                           |
| ----- | -------- | --------------------------------------------- | ----------------------------------- |
| S3-01 | 进入 Step3 | Step2 静态配置校验通过                               | 静态校验失败时留在 Step2，并提示首条错误            |
...
Step4 没有测试。点击「保存」固化当前勾选并记为已保存；保存不改写 Step1～Step3，也不重验工作流。保存与公开均使用蓝底主按钮。点击公开时检查场景主状态是否为可发布；不满足时显示原因并可直接跳转 Step2，满足时显示公开确认；发布成功后生成新版本号并标记已发布。
...
| S4-04 | 公开       | 主状态为可发布                                                                                               | 未满足时提示完成 Step2 设置检查，按钮「Step2へ」直接跳转；满足时显示公开确认弹窗；Step4 是否保存不阻断发布                  |

#### 6.01.10 步骤跳转、引用保护与联动

Step1 提供关联检查；Step2 负责工作流画布配置；Step3 保存通知配置；Step4 保存输出勾选并发起公开。Step2 保存或进入下一步时完成静态配置校验；通过后场景变为可发布。点击 Step4 的公开后显示确认弹窗，确认后直接公开。
...
| Step2 | Step3 | Step2 静态配置校验通过 | 校验通过后保存画布并进入 Step3；不执行样例测试或节点调用             |
| Step3 | Step4 | 点击「次へ」或已保存后继续  | Step3 保存的是通知规则模板；Step4 读取 Step2 OCR 抽出候选    |
| Step4 | 公開    | 当前草稿为可发布        | 显示公开确认；确认后读取 Step1/Step2/Step3/Step4 的当前配置生成新版本             |
...
| 场景           | 条件                    | 用户提示 / 处理                                                  |
| ------------ | --------------------- | ---------------------------------------------------------- |
| Step2 静态校验失败   | 点击 Step2 保存或下一步          | 留在 Step2，提示首条错误并定位对应节点；不保存，场景保持草稿                            |
...
| 状态不可发布       | 场景不是可发布且点击公开          | 标题「現在のステータスでは公開できません。」；提示「Step2の設定チェックを完了し、ステータスを「公開可能」にしてください。」；按钮「Step2へ」「キャンセル」 |
...
7. 静态校验：Step2 保存和下一步执行结构、连线、节点配置、变量引用、字段选择与依赖校验。失败时不保存、不进入 Step3；通过后保存并将场景更新为可发布。校验不读取样例、不执行节点、不创建人工待办、不发送通知或邮件。
...
| 保存        | 保存           | 画布        | 有编辑权限           | 执行静态配置校验；通过后保存节点与连线，失败时留在 Step2 并定位错误             |
| 前へ        | 前へ           | 向导        | 始终可用            | 返回上一步                                            |
| 次へ        | 次へ           | Step2     | 静态配置校验通过       | 保存 Step2 并进入 Step3；失败时留在 Step2 并定位错误                |
| 公開        | 公開           | Step4     | 当前草稿为可发布       | 显示公开确认，确认后新增发布版本并切换到该只读版本                       |
...
Step2 点击保存或下一步时执行静态配置校验。校验只读取当前画布、Step1 账票与字段定义；不读取样例数据，不执行 OCR、AI 检证、数据映射或自定义 JavaScript，不创建人工待办，不发送通知或邮件。
...
校验失败时不保存、不进入 Step3，场景保持草稿。校验通过时保存 Step2 当前配置，场景更新为可发布；点击下一步时继续进入 Step3。Step1 或 Step2 后续发生实质修改时，场景重新变为草稿，必须再次通过静态配置校验并保存。

公开不再执行测试。Step4 的公开按钮仅在当前草稿为可发布时启用；点击后显示公开确认弹窗，确认后直接生成发布版本并切换为只读。

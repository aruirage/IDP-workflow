...
| 清空   | リセット        | Step2   | 已进入 Step2               | 删除所有工作流节点和连线，包括开始节点；测试结论置为未测试；场景回草稿                 |
...
Step2 清空删除所有工作流节点和连线，包括开始节点。开始节点不能单独删除，但 Step2 リセット 会作为整步清空动作一并删除。清空后画布为空，用户需要重新添加开始节点、处理节点、条件节点、人工确认和结束节点后才能测试通过。
...
| 删除节点/连线   | 削除 / この接続を削除 | 选中项       | 有编辑权限；选中项不是开始节点 | 删除选中节点及相关连线；开始节点固定不可删除；删除后置未测试             |
...
画布底部保留两个快捷工具栏：左下角为撤销、取消撤销、变更历史；右下角为缩放、适配画面、添加节点、自动整理。撤销历史只在当前页面打开期间保留，刷新页面或切换场景后重新初始化。
...
| 节点位置编辑 | ノード位置を変更     | 拖动节点位置、自动整理、交换节点位置     |
...
| --------- | ------ | ------ | ---------------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------- |
| 开始节点      | 只读说明   | 否      | 系统固定入口                                               | 不保存配置；单独选中时不显示删除按钮；Step2 リセット 作为整步清空时可一并删除 | 运行时由引擎注入 `caseId`、`files[]`；不进入条件变量池                                        |

...
| Step3 保存成功 | 通知规则模板           | 不改变 Step2 测试结论           | 不改变   | 通知规则为空也可保存；有规则时只校验必填字段和变量有效性       |
...
Step2 提供工作流测试，测试结论是发布门禁，不是进入 Step3 的门禁。测试不是业务用例管理，也不跑真实 OCR、AI、通知或自定义脚本；系统只用内置检查输入验证当前画布配置能否从开始跑到结束。测试未通过也允许进入 Step3/Step4 继续配置，但场景保持草稿且不可发布；画布、节点配置、连线或变量引用发生实质变更后，测试结论作废。
...
| 文件变量     | 开始节点文件列表         | `{files[]}`                                   | 可用于件名或内容说明涉及的文件；不展开文件子字段                                                           |
| 节点输出定义变量 | 可配置通知节点的标准输出变量定义 | `{ocrStatus}`、`{verifyResult}`、`{hitlStatus}` | Step3 为全局模板页，不读取 Step2 当前画布；运行时有对应节点输出时填值，没有值时按缺值规则处理；自定义函数不是通知対象，不提供 `codeStatus` |
...
3. 变量选择器展示系统变量 `caseId`、文件变量 `files[]` 和固定节点类型的标准输出变量定义，不读取 Step2 当前画布，也不随画布节点增删动态变化。
4. 保存时只校验变量 token 格式，不校验变量是否属于当前トリガーイベント，也不校验 Step2 当前画布是否存在対象ノード。
5. 运行时若某变量在具体案件路径中没有值，按空值渲染，并在通知日志记录变量缺值；不阻断工作流。
...
| S3-04 | 变量插入     | 件名和内容可插入系统变量、文件变量和固定节点输出定义变量；变量不改变触发条件            | 变量 token 格式非法时阻断保存并提示变量名            |
| S3-05 | 画布依赖     | Step3 不读取 Step2 当前画布节点实例；通知规则可先作为全局模板保存           | 不因 Step2 没有対象ノード而阻断保存               |
| S3-06 | 内置触发     | 页面不展示触发表达式；处理节点按 `Status / Result` 判断；人工确认按出口事件判断 | 不允许用户改写触发条件                         |
...
| S4-02 | 候选来源     | 侧栏仅展示 Step2 OCR 抽出已开启账票类型；右侧普通字段与表格分区展示；普通字段表和每个表格块都使用字段、OCR 抽出样例、照合结果样例三列；普通字段区和各表格块分别支持拖动排序；允许 0 勾选 | 无 OCR 候选时整页空态；不因此阻断保存                |
...
| Step1 修改匹配策略       | 允许；不破坏引用                                            | 允许；Step2 工作流测试结论置为未测试                                       | 保存成功；检查输入按新策略重新模拟                        |
...
场景卡片只展示名称和操作菜单，不展示状态徽章。Step1～4 顶部工具栏在场景名后同步展示同一状态徽章（下書き / 公開可能 / 公開済み）。状态按草稿、可发布、已发布三类展示。列表工具栏关键词检索，占位「業務シーン名で検索」。无数据时展示空状态；无权限隐藏新建。
...
| 无可选输出字段      | Step4 无 OCR 抽出候选            | 整页缺省：「出力可能な項目がありません」；允许保存空输出设置；不影响发布               |
...
7. 工作流测试：用内置集约完成检查输入做静态校验和模拟路径检查；不跑上传/集约，不跑真实 OCR/AI/通知/自定义脚本，不写正式案件/待办。测试成功后允许发布；测试失败不阻断进入 Step3/Step4，但阻断发布。
...
| 测试弹窗   | 左：检查输入+节点详情；右：检查时间线                   | 孤立/不可达节点不在时间线，画布高亮                              |
...
Step2 画布用工作流测试证明「当前画布配置可跑通」。测试通过是发布门禁，不是进入 Step3 的门禁。测试不是正式跑件，也不是业务测试用例管理：不跑真实 OCR、AI、通知发送或自定义 Python，不创建正式案件、不写真实待办、不检查业务规则是否覆盖所有现实场景。

测试由系统内置检查输入驱动。内置输入只提供一个「案件已集约完成」的只读快照，用来让条件表达式、字段引用和路径模拟有值可算；配置人员不能上传、编辑或替换检查输入。

功能点：

1. 配置人员在 Step2 打开测试弹窗，查看内置检查输入和检查范围后执行测试。
2. 系统按「静态配置校验 → 内置输入适配 → 模拟路径检查 → 发布门禁写入」顺序处理；前一层阻断则不进入后一层。结束节点有无、无出边、可达结束等归结构/模拟维度，不在测试外再挂一层。
3. 模拟路径按内置输入求值后的命中路径生成时间线。条件节点按表达式求值选择分支；人工确认节点不等真人、不建待办，只校验上下文与出口后按可模拟出口继续；任一节点失败立即停止，后续节点保持未执行。
...
flowchart LR
  ...
  staticCheck --> adaptInput["适配内置检查输入"]
  adaptInput --> simulate["模拟命中路径"]
  simulate --> endReach["是否到达结束节点"]
...
| 收起检查输入 | 收起       | 测试弹窗  | 检查输入区已展开    | 折叠左侧检查输入区          |
| 展开检查输入 | 展開       | 测试弹窗  | 检查输入区已收起    | 展开左侧检查输入区          |
...
| 项        | 规则                                                        |
| -------- | --------------------------------------------------------- |
| 输入来源     | 系统内置「标准集约完成案件」快照；不从用户上传文件生成                               |
| 输入数量     | 本期固定 1 个内置检查输入；后续可扩展为多输入，但不影响本期发布门禁口径                     |
| 输入状态     | 视为已完成案件集约，文件已归属到主账票和关联账票；测试只覆盖 Workflow 段                 |
| 输入可编辑性   | 配置人员不能编辑案件编号、文件列表、OCR 样值、标准字段样值、检证样值                      |
| 输入有效性    | 内置输入是系统前提，不作为配置人员测试项；不检查「输入是否存在」「输入是否有文件」                 |
| Step1 适配 | 测试时把内置输入映射到当前 Step1 账票集合；Step1 未配置必要账票或字段时，按当前配置缺失处理为测试失败 |
| 扩展边界     | 本期不提供上传测试 ZIP、不提供用户自定义输入、不保存测试产生的案件实例                     |


怎么测（操作顺序）：

1. 在 Step2 配好画布（至少有开始、结束，以及业务需要的处理节点）。
2. 点工具栏测试按钮打开弹窗；左侧展示内置「标准集约完成案件」检查输入和本次检查范围（只读；本期不开放用户上传替换）。
3. 点弹窗内执行测试。右侧检查时间线按内置输入求值命中路径依次出现状态。
...
| 2   | 连线      | 主输入条数、条件出口、人工确认出口、悬空线                           | 对应节点失败；停止本轮          |
| 3   | 节点配置    | 前处理、OCR 抽出、数据映射、AI 检证、自定义函数等节点的必填配置和引用有效性       | 对应节点失败；停止本轮          |
| 4   | 变量与字段引用 | 条件仅引用可达上游；标准字段/OCR 字段必须来自 Step1；自定义函数入参必须绑定可用变量 | 对应节点失败；停止本轮          |
| 5   | 人工确认规则  | 人工确认最近上游穿透条件节点后必须是前处理、OCR 抽出或 AI 检证；至少有一条有效出口   | 对应节点失败；停止本轮          |
| 6   | 跨模块依赖   | 数据映射设置、AI 检证设置等依赖对象存在且可引用                       | 对应节点失败；停止本轮          |
| 7   | 模拟路径    | 按内置输入求值命中路径，模拟节点状态/结果和分支流转，确认最终可到达结束节点          | 失败节点标原因；其后保持未执行      |
...
| 不检查项      | 原因                                    |
| --------- | ------------------------------------- |
| 真实 OCR 抽出 | OCR 结果来自内置输入，不调用 OCR 服务               |
| 真实 AI 检证  | 只检查检证节点配置与模拟产出，不调用 AI 检证服务            |
| 真实通知发送    | Step3 通知规则不在 Step2 测试中发送              |
| 真实人工确认    | 不创建人工待办，不等待审查者操作                      |
| 自定义脚本执行   | 只检查输入变量绑定与脚本文本存在，不执行 Python           |
| 全分支业务覆盖   | 本期只要求配置结构可跑通；不是证明所有业务情景都被测试数据覆盖       |
| 文件名规则     | Step1 添加账票时已校验基础配置；Step2 测试不再检查文件命名规则 |
...
| 区域      | 展示内容                                  | 说明                        |
| ------- | ------------------------------------- | ------------------------- |
| 左侧检查输入  | 样例名、案件编号、案件标签、文件件数、文件列表、检查范围说明        | 只读；可折叠；文案说明不跑真实 OCR/AI/通知 |
| 左侧节点详情  | 点时间线节点后展示该节点的配置检查结果、模拟状态、失败原因（如有）     | 未执行前展示预览，执行后展示本轮结论        |
| 右侧检查时间线 | 按内置输入求值命中路径的节点顺序；每项有节点名、结果文案、失败原因（如有） | 节点态：成功 / 跳过 / 失败 / 未执行    |
...
| 状态  | 含义                                          |
| --- | ------------------------------------------- |
| 成功  | 该节点通过配置校验，并在模拟路径中可继续向下游流转                   |
| 跳过  | 仅人工确认：测试跳过真实人工确认动作，校验上下文与出口后继续下游；正式运行仍须真人确认 |
| 失败  | 该节点或其前置结构/配置校验未通过，或模拟路径无法继续                 |
...
| 通知   | Step3 通知规则模板     | `caseId`、`files[]`、可配置通知节点的标准输出变量定义（前处理/OCR 抽出/数据映射/AI 检证的 `Status`/`Result`、人工确认 `hitlStatus`） | 触发条件本身不可配置；不得插入标准字段、OCR 字段叶子、Array 容器、Date、DateTime、条件节点输出、自定义函数脚本返回值 | 只用于主题和内容插值；缺值按空值渲染并记日志 |

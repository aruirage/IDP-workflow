...
| 顺序 | 维度 | 检查范围 | 失败处理 |
| -- | -- | ---- | ---- |
| 1 | 结构 | 结束节点、开始到结束可达、分支可达结束、孤立或不可达节点、回流可终止 | 提示首条错误；能对应节点时选中并定位 |
| 2 | 连线 | 同一节点仅允许一条普通输入连线、条件出口不重复、人工确认至少连接一个可连接出口、案件终止不连线、补件只回流、连线两端节点存在 | 同上 |
| 3 | 节点配置 | AI 前处理、OCR 抽出、数据映射、AI 检证、条件、人工确认、自定义 JavaScript 的必填配置及 Step1 账票范围 | 同上 |

...
| 测试失败（终了结构等）  | Step2 测试中无终了、分支不可达终了等 | 在测试结果/时间线中提示对应文案（如「終了ノードを1件以上配置してください」）；不在次へ另做一层校验 |
...
| 审查角色       | 下拉     | 人工确认必填 | 担当者、操作员、操作管理者                     | —            | 只输出 `hitlStatus`；审查动作由三出口表达 |
| 自定义函数      | 入参+脚本  | 脚本必填   | 用户添加的输入参数；每个参数绑定前序节点输出变量          | 脚本返回值不生成下游变量 | 只输出 `codeStatus`            |


开始 / 结束节点面板只读：开始只展示说明：案件集約が完了したときに Workflow を開始します。開始ノードに設定項目はありません。结束只展示说明：Workflow の終点です。設定項目と出力変数はありません。

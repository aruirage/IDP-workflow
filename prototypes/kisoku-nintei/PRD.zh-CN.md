...
| 检查维度 | 检查内容                                                        | 失败处理                     |
| ---- | ----------------------------------------------------------- | ------------------------ |
| 结构   | 结束节点、从固定开始节点到结束的有效路径、孤立节点、回流可终止                             | 保存失败；高亮问题节点或断点；保持草稿 |
| 连线   | 输入数量、条件出口、人工确认出口、悬空连线                                       | 保存失败；提示对应连线或出口问题     |
| 节点配置 | 前处理、OCR 抽出、数据映射、AI 检证、人工确认、自定义函数                            | 保存失败；提示缺失配置或非法依赖     |
| 变量引用 | 条件变量、自定义函数输入、删除或改线后的引用                                      | 保存失败；提示失效变量和节点       |
| 字段选择 | 标准字段和 OCR 字段必须选到叶子字段                                        | 保存失败；提示重新选择字段        |
...
| 场景           | 条件                    | 用户提示 / 处理                                                  |
| ------------ | --------------------- | ---------------------------------------------------------- |
| <mark class="change-modified">Step2 设置检查成功</mark>   | 点击 Step2 设置检查，或在非可发布状态点击公开 | <mark class="change-modified">「設定チェックが完了し、公開可能になりました。」</mark>                            |
| <mark class="change-modified">Step2 设置检查失败</mark>   | <mark class="change-modified">点击 Step2 设置检查，或在非可发布状态点击公开</mark> | <mark class="change-modified">Toast 提示首条错误并定位对应节点；从 Step4 发起时自动跳转 Step2；场景保持草稿</mark>                            |
...
校验失败使用 Warning Toast，不显示确认弹窗。系统按顺序检查；命中失败维度后停止后续维度，只显示该维度第一条错误。错误能对应节点时，自动选中并定位该节点。

校验维度：

| 顺序 | 维度 | 检查范围 | 日文 Toast 文案 | 失败处理 |
| -- | -- | ---- | ---- | ---- |
| 1 | 结构 | 结束节点、开始到结束可达、分支可达结束、孤立或不可达节点、回流可终止 | 「終了ノードを1件以上配置してください」<br>「開始ノードから終了ノードへ到達できません」<br>「分岐「{分支名}」が終了ノードに到達できません」<br>「接続のない孤立ノードがあります」<br>「開始から到達できないノードがあります」<br>「回流により終了ノードへ到達できない経路があります」 | 提示首条错误；能对应节点时选中并定位 |
| 2 | 连线 | 主输入数量、条件出口、人工确认出口、案件终止不连线、补件只回流、悬空端点 | 「主入力は1本までです」<br>「条件出口「{出口名}」が重複しています」<br>「ELSE 出口は1本までです」<br>「人工確認出口を1本以上接続してください」<br>「案件終止出口は接続できません」<br>「補件出口は前述ノードへの回流接続のみ可能です」<br>「人工確認出口「{出口名}」が重複しています」<br>「宙に浮いた接続があります（端点ノードが存在しません）」 | 同上 |
| 3 | 节点配置 | AI 前处理、OCR 抽出、数据映射、AI 检证、条件、人工确认、自定义 JavaScript 的必填配置及 Step1 账票范围 | 「前処理オプションを1件以上有効にしてください」<br>「前処理対象帳票「{账票类型}」が Step1 にありません」<br>「OCR 対象帳票を1件以上有効にしてください」<br>「OCR 対象帳票「{账票类型}」が Step1 にありません」<br>「データマッピング設定が未選択です」<br>「検証モジュールを1件以上有効にしてください」<br>「未設定の条件分岐があります」<br>「審査ロールを選択してください」<br>「JavaScript を入力してください」 | 同上 |
| 4 | 变量引用 | 条件变量和自定义函数输入可从上游到达；删除或改线后无失效引用 | 「条件変数「{变量名}」が上流から到達できません（改線・削除後の無効参照含む）」<br>「関数入力「{变量名}」が上流から到達できません（改線・削除後の無効参照含む）」 | 同上 |
| 5 | 字段选择 | 条件左值选到可判断的叶子字段，字段类型与运算符匹配 | 「条件左値「{变量名}」は容器です。葉項目を選んでください」<br>「条件左値「{变量名}」は条件に使えない型です」 | 同上 |
| 6 | 依赖 | 人工确认合法上游、数据映射上游、AI 检证上游满足节点依赖 | 「{节点名}：条件ノードを挟んだ直前上流が前処理・OCR・AI検証のいずれかになるよう接続してください。現在の上流は {上游节点} です。」<br>「データマッピングの上流に前処理/OCR がありません」<br>「AI検証の上流に OCR/前処理結果がありません」 | 同上 |

<mark class="change-modified">校验失败时场景保持草稿；从 Step4 发起检查时，Toast 提示首条原因并自动跳转 Step2。校验通过时保存当前检查结果，场景更新为可发布；从 Step4 发起检查时继续显示公开确认。Step1 或 Step2 后续发生实质修改时，场景重新变为草稿，必须再次通过设置检查。</mark>

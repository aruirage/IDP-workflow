...
四步之间允许来回跳转。Step1 的关联账票集合和 OCR 字段是 Step2 的账票/字段范围来源；Step2 不单独维护账票类别和 OCR 字段。前处理目标账票、OCR 节点账票行、条件节点中的账票字段、AI 检证账票上下文，都从 Step1 派生。用户不能直接做会破坏后续引用的修改；系统在保存当前步骤前检查后续步骤引用关系，如果存在引用，阻断本次修改，并提示用户先到后续步骤解除引用。
...
| 删除节点/连线   | 削除 / この接続を削除 | 选中项       | 有编辑权限；选中项不是开始节点 | 删除选中节点及相关连线；开始节点只能通过 Step2 リセット 一并清空；删除后置未测试 |
...
| --------- | ------ | ------ | ---------------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------- |
| 开始节点      | 只读说明   | 否      | 系统固定入口                                               | 不保存配置；单独选中时不显示删除按钮   | 运行时由引擎注入 `caseId`、`files[]`；不进入条件变量池；Step2 リセット 可清空整张画布                                        |
...
| 审查角色      | 下拉     | 人工确认必填 | 担当者、操作员、操作管理者                                        | —                    | 保存为人工确认待办指派规则；运行时生成待办并输出 `hitlStatus`；完成、补件、案件终止由三条画布出口表达，Step3 通知也按这三条出口触发                    |
| 自定义函数输入参数 | 弹窗+列表  | 按用户添加  | 用户添加的输入参数；每个参数绑定开始节点 `files[]`、文件类别 JSON 变量或前序节点输出变量 | 参数名唯一；Source 不可选后续节点 | 保存为 Python 节点入参映射；脚本内用自定义变量名读取入参；脚本返回值只进执行日志，不进变量池                                             |
| 自定义函数脚本   | 代码编辑器  | 是      | 用户输入 Python 脚本                                       | 脚本为空阻断保存；运行时异常写执行日志  | 保存为 Python 节点执行脚本；运行时只输出 `codeStatus`，条件节点可消费该状态，Step3 通知不把自定义函数作为対象ノード                        |
| 结束节点      | 只读说明   | 否      | 系统固定终点                                               | 不能有出边                | 不保存配置，不输出变量；到达结束仅代表工作流路径结束，案件状态由平台状态机汇聚                                                        |
...
| 通知   | Step3 通知规则模板     | `caseId`、`files[]`、可配置通知节点的标准输出变量定义（前处理/OCR 抽出/数据映射/AI 检证的 `Status`/`Result`） | 触发条件本身不可配置；自定义函数脚本返回值                               | 只用于主题和内容插值；缺值按空值渲染并记日志 |
| 系统   | 引擎、日志、状态汇聚、Step4 | 开始 `caseId` / `files[]`、映射 `standardFields` 等                                 | 不提供给条件                                              | 可在出力変数面板展示             |
...
| 数据映射节点 | `standardFields` | Array  | 系统  | 标准字段对象数组；条件只能选叶子 |

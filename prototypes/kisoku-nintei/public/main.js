...
const appOptions = {
  setup() {
    ...
    const UI_ZH_REPLACEMENTS = [
      ...
      ['ファイル名生成で使用する入力名と参照元を定義します。上流変数を複製せず、参照元が変わると無効になります。', '定义文件名生成中使用的输入名和引用源。不会复制上游变量，引用源变化时会失效。'],
      ['出力変数', '输出变量'],
      ...
      ['ファイル名の戻り値は実行結果として保存します。', '文件名返回值会作为执行结果保存。'],
      ['入力変数がありません', '没有输入变量'],
      ['出力変数がありません', '没有输出变量'],
      ['変数名', '变量名'],
      ['ソース', '来源'],
      ...
      ['条件を追加', '添加条件'],
      ['前処理オプション', '前处理选项'],
      ['画像回転・補正・画像整列など、OCR 前の処理を有効化します。', '启用 OCR 前的图像旋转、补正、图像排列等处理。'],
      ['対象帳票', '对象账票'],
      ['分類・保管のみ', '仅分类・保管'],
      ...
      ['公開前チェック（本テストでは未実行）', '发布前检查（本测试未执行）'],
      ['案件集約：複数候補', '案件集约：多候选'],
      ['集約要確認', '集约待确认'],
      ...
      ['条件分岐ノードに設定未完了の条件式があります', '条件判断节点中存在配置未完成的条件表达式'],
      ...
      ['跨批次 + 人工缓冲', '跨批次 + 人工缓冲'],
      ['内蔵シナリオで Workflow をテスト実行します。右側の時間軸と左側のノード詳細で結果を確認します。', '使用内置场景执行 Workflow 测试。通过右侧时间轴和左侧节点详情确认结果。'],
      ['关注点', '关注点'],
      ...
      ['画像補正・回転・画像整列を完了しました', '已完成图像补正・旋转・图像排列'],
